...
class TokenManager {
    ...
    public function __construct() {
        $this->secretKey = hash('sha256', 'KW_AZMOON_ACCESS_TOKEN_SECRET_' . date('Y-m-d'));
        $this->refreshSecretKey = hash('sha256', 'KW_AZMOON_REFRESH_TOKEN_SECRET_' . date('Y-m-d'));
    }
...
class KwAzmoonDatabase {
    ...
    public function getKwAzmoonData($page = 1, $pageSize = 10, $filters = []) {
        ...
        $countQuery = "SELECT COUNT(*) as total FROM Kw_Azmoon $whereClause";
    ...
    // درج رکورد جدید
    public function insertKwAzmoonRecord($data) {
        $columns = array_keys($data);
        $placeholders = array_map(function($col) { return ":$col"; }, $columns);

        $query = "INSERT INTO Kw_Azmoon (" . implode(", ", $columns) . ") 
    ...
    // به‌روزرسانی رکورد
    public function updateKwAzmoonRecord($id, $data) {
        $updateFields = array_map(function($col) { return "[$col] = :$col"; }, array_keys($data));
        $query = "UPDATE Kw_Azmoon SET " . implode(", ", $updateFields) . " WHERE Id = :id";
    ...
    // حذف رکورد
    public function deleteKwAzmoonRecord($id) {
        $query = "DELETE FROM Kw_Azmoon WHERE Id = :id";
        $stmt = $this->pdo->prepare($query);
    ...
    public function getLatestRecords($limit = 10) {
        ...
        $query = "SELECT TOP " . (int)$limit . " * FROM Kw_Azmoon ORDER BY Id DESC";
        $stmt = $this->pdo->prepare($query);
        $stmt->execute();
...
class KwDskWebServiceAzmoon {
    ...
    public function __construct() {
        try {
            ...
            $this->database = new KwAzmoonDatabase();
    ...
    private function handleGetRequest() {
        try {
            ...
            if (isset($_GET['action']) && $_GET['action'] === 'latest_records') {
                ...
                $result = $this->database->getKwAzmoonData($page, $pageSize, $filters);
            }
    ...
    private function handlePostRequest() {
        try {
            ...
            $result = $this->database->insertKwAzmoonRecord($data);
    ...
    private function handlePutRequest() {
        try {
            ...
            $result = $this->database->updateKwAzmoonRecord($data['Id'], $data);
    ...
    private function handleDeleteRequest() {
        try {
            ...
            $result = $this->database->deleteKwAzmoonRecord($id);
