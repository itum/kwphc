...
class University_Management {
    ...
    public function register_shortcodes() {
        ...
        // خروجی JSON کلاس‌ها با فیلتر تاریخ و روز هفته
        add_shortcode('um_classes_json', array($this, 'classes_json_shortcode'));
    }

    /**
     * شورت‌کد JSON کلاس‌ها با فیلتر تاریخ و روز هفته
     * [um_classes_json from="2025-09-01" to="2025-09-30" weekdays="6,0,1" fullweek="0"]
     */
    public function classes_json_shortcode($atts) {
        ...
        $weekdays = array();
        if (!empty($atts['weekdays'])) {
            foreach (explode(',', $atts['weekdays']) as $wd) {
                $wd = intval(trim($wd));
                if ($wd >= 0 && $wd <= 6) { $weekdays[] = $wd; }
            }
        }
        $fullweek = $atts['fullweek'] === '1' || $atts['fullweek'] === 'yes';
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) { $query->the_post();
                ...
                $duration = intval(get_post_meta(get_the_ID(), '_class_duration', true));
                $teacher = get_post_meta(get_the_ID(), '_class_teacher', true);
                $status = get_post_meta(get_the_ID(), '_class_status', true) ?: 'scheduled';
                $classes[] = array(
                    'id' => get_the_ID(),
                    'name' => get_the_title(),
        ...
        // خروجی به صورت pre برای مشاهده
        ob_start();
        echo '<pre class="um-classes-json" style="white-space:pre-wrap;direction:ltr">' . esc_html(wp_json_encode($classes, JSON_UNESCAPED_UNICODE)) . '</pre>';
        return ob_get_clean();
    }
